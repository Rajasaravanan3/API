...
//             loadDetails();
//         }
//         else if(!localStorage_obj.data.length){
//             localStorage.setItem("user_array",this.responseText);
//             obj = (JSON.parse(localStorage.getItem("user_array"))).data;
//             records_count.innerHTML ="0";
//                 tableBody.innerHTML = `<tr><td colspan="5"><img src="./svg images/no-data-found.svg" alt="No record found" width="70%"></td></tr>`;
...
//         text+=`<td class="user" contenteditable="true">${obj[index].first_name}</td>`;
//         text+=`<td class="user" contenteditable="true">${obj[index].last_name}</td>`;
//         text+=`<td class="user" contenteditable="true">${obj[index].email}</td>`;
//         text+=`<td class="change"><button class="save">Save</button><button class="delete" onclick ="removeUser(${index})">Delete</button></td></tr>`;
//     }
